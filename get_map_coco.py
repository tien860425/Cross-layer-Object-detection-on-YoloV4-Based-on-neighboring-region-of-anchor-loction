...
GT_PATH = os.path.join(os.getcwd(), 'input_coco', 'ground-truth')
DR_PATH = os.path.join(os.getcwd(), 'input_coco', 'detection-results')
# if there are no images then no animation can be shown
IMG_PATH = os.path.join(os.getcwd(), 'input_coco', 'images-optional')
...
results_files_path = "results_coco"
...
with open(results_files_path + "/results.txt", 'w') as results_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        F1 = 2*np.array(rec)*np.array(prec)/(np.array(prec)+np.array(rec)+1e-10)
...
"""
with open(results_files_path + "/results_coco.txt", 'a') as results_file:
